<template>
  <div class="grid grid-cols-12 gap-4 p-3">
    <div class="col-span-12">
      <span class="font-bold text-xl">Типы организации здравоохранения</span>
    </div>
    <div class="col-span-12" v-if="AdminService.hasAccess(['TYPEOZ_CREATE'])">
  ...
  <div class="box p-3 rounded-xl">
    <div class="grid grid-cols-12 gap-4 p-2">
      <div class="col-span-12 lg:col-span-3 md:col-span-4">
        <span class="font-medium">Кол-во: </span> {{ paginationParams.totalItems }}
      </div>
    </div>
    <div class="preview">
              ...
              <div class="dropdown">
                ...
                <div class="dropdown-menu">
                  <div class="dropdown-menu__content box dark:bg-dark-1 p-2">
                    <button v-if="AdminService.hasAccess(['TYPEOZ_EDIT'])"
                            @click="showModalEdit(item)"
                            class="flex items-center block p-2 transition duration-300 ease-in-out bg-white dark:bg-dark-1 hover:bg-gray-200 dark:hover:bg-dark-2 rounded-md">
                      <Edit2Icon class="w-4 h-4 mr-2"/>
                      Редактировать
                    </button>
                    <router-link v-if="AdminService.hasAccess(['TYPEOZ_EDIT'])"
                                 @click="hide"
                                 :to="{name: 'TypeOrganizationSetRoles', query: {id: item.id, name: item.name}}"
                                 class="flex items-center block p-2 transition duration-300 ease-in-out bg-white dark:bg-dark-1 hover:bg-gray-200 dark:hover:bg-dark-2 rounded-md">
                      <Edit2Icon class="w-4 h-4 mr-2"/>
                      Установить роли
                    </router-link>
                    <a v-if="AdminService.hasAccess(['TYPEOZ_DELETE'])"
                       class="flex items-center block p-2 transition duration-300 ease-in-out bg-white dark:bg-dark-1 hover:bg-gray-200 dark:hover:bg-dark-2 rounded-md"
                       href="javascript:;"
                       data-toggle="modal"
...
import {AdminService, ProjectService, ServicesService, TypeOrganizationService} from "@/services";
...
function getByPagination(pageNumber) {
  ...
  TypeOrganizationService.getList(pageNumber, paginationParams.pageSize).then(response => {
...
function create() {
  ...
  TypeOrganizationService.create(modalParams.datas).then(response => {
    createToast({
...
function update() {
  modalParams.loading = true
  TypeOrganizationService.edit(modalParams.datas).then(response => {
...
function confirmDelete() {
  TypeOrganizationService.remove(itemForDelete.value).then(response => {
    createToast({
