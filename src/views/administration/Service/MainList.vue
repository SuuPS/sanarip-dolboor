<template>
  <div class="grid grid-cols-12 gap-4 p-3">
    <div class="col-span-12">
      <span class="font-bold text-xl">Сервисы</span>
    </div>
    <!-- ФИЛЬТР-->
    <div class="col-span-12" v-if="AdminService.hasAccess(['SERVICE_CREATE'])">
        ...
        <table class="table">
          ...
          <tr v-for="item in paginationParams.result" v-bind:key="item">
            <td class="border text-center">
              <img v-if="String(item.fileLOGO).includes('.jpeg') || String(item.fileLOGO).includes('.jpg') || String(item.fileLOGO).includes('.png')" :src="'https://dev.med.kg/imed-back/uploads/'+item.fileLOGO" alt="logo">
            </td>
            <td class="border text-center">
              {{ item.name }}
            </td>
              ...
              <div class="dropdown">
                ...
                <div class="dropdown-menu">
                  <div class="dropdown-menu__content box dark:bg-dark-1 p-2">
                    <button v-if="AdminService.hasAccess(['SERVICE_EDIT'])"
                                 @click="showModalEdit(item)"
                                 class="flex items-center block p-2 transition duration-300 ease-in-out bg-white dark:bg-dark-1 hover:bg-gray-200 dark:hover:bg-dark-2 rounded-md">
                      <Edit2Icon class="w-4 h-4 mr-2"/>
                      Редактировать
                    </button>
                    <a v-if="AdminService.hasAccess(['SERVICE_DELETE'])"
                       class="flex items-center block p-2 transition duration-300 ease-in-out bg-white dark:bg-dark-1 hover:bg-gray-200 dark:hover:bg-dark-2 rounded-md"
                       href="javascript:;"
                       data-toggle="modal"
      ...
      <div class="modal-content">
        ...
        <form v-on:submit.prevent="modalParams.isEdit ? update() :create()">
          <div class="modal-body grid grid-cols-12 gap-4 gap-y-3">
            <div class="col-span-12">
              <img class="w-1/3 rounded-xl shadow" v-if="modalParams.datas.preview != null" :src="modalParams.datas.preview" alt="">
              <img class="w-1/4 rounded-xl shadow" v-else-if="modalParams.datas.fileLOGO != null" :src="'https://dev.med.kg/imed-back/uploads/'+modalParams.datas.fileLOGO" alt="">
            ...
            <div class="col-span-12">
              ...
              <textarea
                required
                v-model="modalParams.datas.description"
...
import {AdminService, ServicesService} from "@/services";
...
function getByPagination(pageNumber) {
  ...
  ServicesService.getList(pageNumber, paginationParams.pageSize).then(response => {
...
function showModalEdit(item) {
  ...
  modalParams.datas.fileLOGO = item.fileLOGO
}

function create() {
  modalParams.loading = true
  ServicesService.create(modalParams.datas).then(response => {
    createToast({
...
function update() {
  modalParams.loading = true
  ServicesService.edit(modalParams.datas).then(response => {
    createToast({
      title: 'Запись успешно сохранена!'
    }, {
      type: 'success',
...
function resetData() {
  ...
  modalParams.datas.preview = null
  modalParams.datas.fileLOGO = null
  modalParams.datas.formFileLOGO = null
}
...
function confirmDelete() {
  ServicesService.remove(itemForDelete.value).then(response => {
    createToast({
...
function handleFileUpload(event) {
  ...
  modalParams.datas.formFileLOGO = event.target.files[0];
}
