<template>
  <div class="grid grid-cols-12 gap-4 mt-5">
    <div class="col-span-full"><h1 class="text-xl font-bold">Страны</h1></div>
      ...
      <div class="overflow-auto lg:overflow-visible mt-8 sm:mt-0">
        <table class="table sm:mt-2 bg-white rounded-xl">
          ...
          <tr v-for="codeitem in genResult.content" :key="codeitem" class="hover:bg-gray-200">
            ...
            <td class="border-b dark:border-dark-5">{{ codeitem.codeISO.substr(0, 50) }}</td>
      ...
      <div class="modal-content">
        ...
        <form v-on:submit.prevent="modalParams.isEditModal ? edit() : create()">
          <div class="modal-body grid grid-cols-12 gap-4 gap-y-3">
            ...
            <div class="col-span-12">
              <label class="form-label">ISO Код<span style="color: red">*</span></label>
              <input
                maxlength="255"
                v-model="modalParams.datas.codeISO"
                required
                type="text"
...
async function getByPagination(page) {
  try {
    await (async () => {
      CountryService.getCountryWithPagination({page: page, size: 15}).then(response => {
...
function showEditModal(item) {
  ...
  modalParams.datas.codeISO = item.codeISO
}

function confirmDelete() {
  CountryService.deleteCountry({id: idCode.value}).then(response => {
    createToast({
...
function edit() {
  loading.value = true
  CountryService.editCountry(modalParams.datas).then(response => {
    loading.value = false
    createToast({
...
function create() {
  loading.value = true
  CountryService.createCountry(modalParams.datas).then(response => {
    loading.value = false
    createToast({
...
function resetData() {
  ...
  modalParams.datas.codeISO = null
}
