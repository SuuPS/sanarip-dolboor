<template>
  <div class="grid grid-cols-12 gap-4 mt-5">
    <div class="col-span-full"><h1 class="text-xl font-bold">Сроки годности</h1></div>
...
async function getByPagination(page) {
  try {
    await (async () => {
      DurationService.getDurationWithPagination({page: page, size: 15}).then(response => {
...
function confirmDelete() {
  DurationService.deleteDuration({id: idCode.value}).then(response => {
    createToast({
...
function edit() {
  loading.value = true
  DurationService.editDuration(modalParams.datas).then(response => {
    loading.value = false
    createToast({
...
function create() {
  loading.value = true
  DurationService.createDuration(modalParams.datas).then(response => {
    loading.value = false
    createToast({
