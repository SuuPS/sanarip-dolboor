<template>
  <div class="grid grid-cols-12 gap-4 mt-5">
    <div class="col-span-full"><h1 class="text-xl font-bold">Отделы</h1></div>
    <div class="col-span-12 border rounded-xl p-3 bg-theme-4 shadow">
      <div class="flex justify-between">
        <button v-if="checkArray('ADMIN')"
          type="button"
        ...
        <Select2
          :disabled="checkArray('ADMIN') == false"
          style="width: 200px;"
          required
          @select="onChangeOraganizations()"
          :options="listOrganization"
          :settings="select2Settings"
          v-model="filter.filterByOrganizationId"/>
      </div>
      <div class="overflow-auto lg:overflow-visible mt-8 sm:mt-0">
        <table class="table sm:mt-2 bg-white rounded-xl">
          ...
          <tr v-for="codeitem in genResult.content" :key="codeitem" class="hover:bg-gray-200">
            ...
            <td class="border-b dark:border-dark-5">{{ codeitem.organization.ozFullName.substr(0, 50) }}</td>
...
import {CountryService, DepartmentService, OrganisationService} from "@/services";
...
async function getByPagination(page) {
  try {
    await (async () => {
      DepartmentService.getDepartmentWithPagination({page: page, size: 15, filterByOrganizationId: filter.filterByOrganizationId}).then(response => {
...
function confirmDelete() {
  DepartmentService.deleteDepartment({id: idCode.value}).then(response => {
    createToast({
...
function edit() {
  loading.value = true
  DepartmentService.editDepartment(modalParams.datas).then(response => {
    loading.value = false
    createToast({
...
function create() {
  loading.value = true
  DepartmentService.createDepartment(modalParams.datas).then(response => {
    loading.value = false
    createToast({
...
onMounted(async () => {
  if (checkArray('ADMIN') == false) filter.filterByOrganizationId = user.organization.id

  await getByPagination(0)
