<template>
  <div class="grid grid-cols-12 gap-4 mt-5">
    <div class="col-span-full"><h1 class="text-xl font-bold">Коды Тн Вэд</h1></div>
        ...
        <ul class="pagination">
          ...
          <template v-for="page in genResult.pagenumbersEnd" v-bind:key='page'>
            <li v-if="genResult.pagenumbersStart <= page || page >= genResult.pagenumbersEnd">
              <a class="pagination__link"
                 v-bind:class="{ 'pagination__link--active' : page === genResult.number + 1}"
...
import {CodeTnVedService} from "@/services";
...
async function getByPagination(page) {
  try {
    await (async () => {
      CodeTnVedService.getCodeProductsWithPagination({page: page, size: 15}).then(response => {
...
function confirmDelete() {
  CodeTnVedService.deleteVed({id: idCode.value}).then(response => {
    createToast({
...
function edit() {
  loading.value = true
  CodeTnVedService.editVed(modalParams.datas).then(response => {
    loading.value = false
    createToast({
...
function create() {
  loading.value = true
  CodeTnVedService.createVed(modalParams.datas).then(response => {
    loading.value = false
    createToast({
