<template>
  <div class="grid grid-cols-12 gap-4 mt-5">
    <div class="col-span-full"><h1 class="text-xl font-bold">Должности</h1></div>
...
import {PositionWorkService} from "@/services";
...
async function getByPagination(page) {
  try {
    await (async () => {
      PositionWorkService.getPositionWorkWithPagination({page: page, size: 15}).then(response => {
...
function confirmDelete() {
  PositionWorkService.deletePositionWork({id: idCode.value}).then(response => {
    createToast({
...
function edit() {
  loading.value = true
  PositionWorkService.editPositionWork(modalParams.datas).then(response => {
    loading.value = false
    createToast({
...
function create() {
  loading.value = true
  PositionWorkService.createPositionWork(modalParams.datas).then(response => {
    loading.value = false
    createToast({
