<template>
  <div class="grid grid-cols-12 gap-4 mt-5">
    <div class="col-span-full"><h1 class="text-xl font-bold">Виды товара</h1></div>
...
import { VidTovara } from "@/services";
...
async function getByPagination(page) {
  try {
    await (async () => {
      VidTovara.getVidTovaraWithPagination({page: page, size: 15}).then(response => {
...
function confirmDelete() {
  VidTovara.deleteVidTovara({id: idCode.value}).then(response => {
    createToast({
...
function edit() {
  loading.value = true
  VidTovara.editVidTovara(modalParams.datas).then(response => {
    loading.value = false
    createToast({
...
function create() {
  loading.value = true
  VidTovara.createVidTovara(modalParams.datas).then(response => {
    loading.value = false
    createToast({
