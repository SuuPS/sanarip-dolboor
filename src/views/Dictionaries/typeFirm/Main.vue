...
async function getByPagination(page) {
  try {
    await (async () => {
      TypeFirmService.getTypeFirmWithPagination({page: page, size: 15}).then(response => {
...
function confirmDelete() {
  TypeFirmService.deleteTypeFirm({id: idCode.value}).then(response => {
    createToast({
...
function edit() {
  loading.value = true
  TypeFirmService.editTypeFirm(modalParams.datas).then(response => {
    loading.value = false
    createToast({
...
function create() {
  loading.value = true
  TypeFirmService.createTypeFirm(modalParams.datas).then(response => {
    loading.value = false
    createToast({
