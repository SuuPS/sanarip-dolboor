<template>
  <div class="grid grid-cols-12 gap-4 mt-5">
    <div class="col-span-full"><h1 class="text-xl font-bold">Организации</h1></div>
      ...
      <div class="overflow-auto lg:overflow-visible mt-8 sm:mt-0">
        <table class="table sm:mt-2 bg-white rounded-xl">
          ...
          <tr v-for="codeitem in genResult.content" :key="codeitem" class="hover:bg-gray-200">
            <td class="border-b dark:border-dark-5">{{ codeitem.ozFullName != '' && codeitem.ozFullName != null ? codeitem.ozFullName.substr(0, 50) : '(отсутствует)' }}</td>
            <td class="border-b dark:border-dark-5">{{ codeitem.ozCode != '' && codeitem.ozCode != null ? codeitem.ozCode.substr(0, 50) : '(отсутствует)' }}</td>
            <td class="border-b dark:border-dark-5">{{ codeitem.ozShortName != '' && codeitem.ozShortName != null ? codeitem.ozShortName.substr(0, 50) : '(отсутствует)' }}</td>
            <td class="border-b dark:border-dark-5">{{ codeitem.country.name.substr(0, 50) }}</td>
        ...
        <form v-on:submit.prevent="modalParams.isEditModal ? edit() : create()">
          <div class="modal-body grid grid-cols-12 gap-4 gap-y-3">
            ...
            <div class="col-span-12">
              ...
              <input
                maxlength="255"
                v-model="modalParams.datas.ozFullName"
                required
                type="text"
            ...
            <div class="col-span-12">
              <label class="form-label">Краткое название <span style="color: red">*</span></label>
              <input
                maxlength="255"
                v-model="modalParams.datas.ozShortName"
                required
                type="text"
                placeholder="Введите название..."
                class="form-control"
            ...
            <div class="col-span-12">
              ...
              <input
                v-model="modalParams.datas.ozCode"
                required
                type="text"
            ...
            <div class="col-span-12">
              <label class="form-label">Выберите страну <span style="color: red">*</span></label>
              <Select2
                required
                :options="listCountry"
...
async function getByPagination(page) {
  try {
    await (async () => {
      OrganisationService.getOrganizationWithPagination({page: page, size: 15}).then(response => {
...
function showEditModal(item) {
  ...
  modalParams.datas.ozCode = item.ozCode
  modalParams.datas.ozFullName = item.ozFullName
  modalParams.datas.ozShortName = item.ozShortName
  modalParams.datas.country.id = item.country.id
  modalParams.datas.count.id = item.count.id
}

function confirmDelete() {
  OrganisationService.deleteOrganization({id: idCode.value}).then(response => {
    createToast({
...
function edit() {
  loading.value = true
  OrganisationService.editOrganization(modalParams.datas).then(response => {
    loading.value = false
    createToast({
...
function create() {
  loading.value = true
  OrganisationService.createOrganization(modalParams.datas).then(response => {
    loading.value = false
    createToast({
...
function resetData() {
  ...
  modalParams.datas.ozCode = null
  modalParams.datas.ozFullName = null
  modalParams.datas.ozShortName = null
  modalParams.datas.country.id = null
  modalParams.datas.count.id = null
}
